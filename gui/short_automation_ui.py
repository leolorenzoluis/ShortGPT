...
def create_short_automation_ui():
    with gr.Row(visible=False) as short_automation:
        with gr.Column():
            ...
            short_type = gr.Radio(["Reddit Story shorts", "Historical Facts shorts", "Scientific Facts shorts",
                                  "Custom Facts shorts"], label="Type of shorts generated", value="Custom Facts", interactive=True)
            facts_subject = gr.Textbox(
                label="Write a subject for your facts (example: Football facts)", interactive=True, visible=False)
            short_type.change(lambda x: gr.update(visible=x == "Custom Facts shorts"), [
                              short_type], [facts_subject])
            language = gr.Radio(
                language_choices, label="Language", value="ENGLISH")

            useImages = gr.Checkbox(label="Use images", value=True)
            numImages = gr.Radio(
                [5, 10, 25], value=25, label="Number of images per short", visible=True, interactive=True)
            useImages.change(lambda x: gr.update(
                visible=x), useImages, numImages)

            addWatermark = gr.Checkbox(label="Add watermark")
            watermark = gr.Textbox(
                label="Watermark (your channel name)", visible=False)
            addWatermark.change(lambda x: gr.update(
                visible=x), [addWatermark], [watermark])
        ...
        if platform.system() == "Windows":
            video_folder.click(lambda _: os.startfile(
                os.path.abspath("videos/")))
        else:
            video_folder.click(lambda _: os.system(
                f"open {os.path.abspath('videos/')}"))
...
def inspect_create_inputs(
    ...
    if watermark != "":
        if not watermark.replace(" ", "").isalnum():
            raise gr.Error(
                "Watermark should only contain letters and numbers.")
        if len(watermark) > 15:
...
def update_progress(progress, progress_counter, num_steps, num_shorts, stop_event):
    ...
    while not stop_event.is_set():
        ...
        progress(progress_counter / (num_steps * num_shorts),
                 f"Step 12 in progress - {dynamic}/3649")
...
def create_short_engine(short_type, language, numImages,
    ...
    if "fact" in short_type.lower():
        ...
        return FactsShortEngine(facts_type=facts_subject, background_video_name=background_video,
                                ...
                                language=language)
    raise gr.Error(
        f"Short type does not have a valid short engine: {short_type}")
...
def create_short(numShorts,
    ...
    background_videos = (background_video_list *
                         ((numShorts // len(background_video_list)) + 1))[:numShorts]
    background_musics = (background_music_list *
                         ((numShorts // len(background_music_list)) + 1))[:numShorts]
    ...
    try:
        for i in range(numShorts):
            ...
            def logger(prog_str):
                progress(progress_counter / (num_steps * numShorts),
